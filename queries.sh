...
echo "$($PSQL "SELECT DISTINCT(name) FROM games JOIN teams on games.winner_id = teams.team_id WHERE year = 2014 AND round = 'Eighth-Final' UNION SELECT DISTINCT(name) FROM games JOIN teams ON games.opponent_id = teams.team_id WHERE year = 2014 AND round = 'Eighth-Final' ORDER BY name")"
